...
test_software_detect (gconstpointer user_data)
{
	...
	for (i_step = 0; i_step < 5; i_step++) {
		...
		case NM_LINK_TYPE_GRETAP: {
			...
			g_assert_cmpint (plnk->input_key, ==, 0);
			g_assert_cmpint (plnk->output_key, ==, 0);
			nmtst_assert_ip4_address (plnk->local, "192.168.1.133");
			nmtst_assert_ip4_address (plnk->remote, "172.168.101.2");
			g_assert_cmpint (plnk->ttl, ==, 39);
			g_assert_cmpint (plnk->tos, ==, 12);
